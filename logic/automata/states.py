# -*- coding: utf-8 -*-
from logic.graph.nodes import DirectedWeightedNode,Node
...
class CellularState(DirectedWeightedNode):
	...
	def get_new_state(self, path): #get the state that is reached from this node, following the $path
		...
		for (next_state,upper_range) in self.adjacencies(): # l lista adjacencies tiene duplas (adjacency, weight) = (adyacencia,ponderacion)
			if path[0] <= upper_range:
